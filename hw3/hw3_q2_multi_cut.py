...
def ModifyAndSolveSP(k):
    ...
    CutFound = False
    if(nsol[k] < SPobj - CutViolationTolerance): # Found Benders cut is violated at the current master solution
        CutFound = True
...
n = MP.addVars(scenarios, obj=prob, name='n') #the Q values are always non-negative according to the problem
...
while (CutFound) and (NoIters<=199):
    ...
    nsol = dict(zip(scenarios, [n[k].x for k in scenarios]))

    UB = sum([theta[i]*xsol[i] for i in cities])
    
    n_cut = 0

    for k in scenarios:
        Qvalue, CutFound_k, pi_sol, gamma_sol = ModifyAndSolveSP(k)
        ...
        if(CutFound_k):
            CutFound = True
            n_cut += 1
            expr = LinExpr(n[k] - (x.sum()-I)*pi_sol - quicksum((demand[i,k]-Yn[i]-x[i])*gamma_sol[i] for i in cities))
            MP.addConstr(expr >= 0)
